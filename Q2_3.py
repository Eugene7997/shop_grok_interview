...
pattern_matching = r"(?<=products_storage=\s\[)+(.*)+(?=];$)"
desired_string = re.search(pattern_matching, product_storage_string_raw, re.DOTALL|re.MULTILINE)[0]
desired_string_replaced = re.sub(r'\r\n|\r|\n', ' ', desired_string)
product_storage_json = json.loads(json.dumps(desired_string_replaced))
